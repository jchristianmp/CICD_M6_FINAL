...
predObj = classPrediction()

# Título
st.markdown("""
            <h1 style='text-align: center;
            color: white;'>Predicción de Precios de Carros</h1>""",
...
st.markdown("""<h2 style='text-align: center;
            color: white;'>Christian Matos</h2>""",
            unsafe_allow_html=True)
...
css_carga_archivo = '''
                    ...
                    { content: "Suba o arraste el archivo aqui"}
                    [data-testid="stFileUploaderDropzone"] div div span
                    { display:none;}
                    [data-testid="stFileUploaderDropzone"] div div::after
                    {font-size: .8em; content:"Límite: 200MB por archivo"}
...
if uploaded_file is not None:
    ...
    df_datos_subidos = pd.read_csv(uploaded_file)

    # Muestra contenido de csv cargado
    st.write('Contenido del archivo cargado:')
    st.dataframe(df_datos_subidos)
    ...
    if st.button("click aqui para enviar el CSV al Pipeline"):
        if uploaded_file is None:
            ...
            with st.spinner('Pipeline y Modelo procesando...'):
                ...
                pred, pred_sinesc, d_proc = predObj.pred_inf(df_datos_subidos)
                time.sleep(5)
                st.success('Listo!')

                # Concatenación de predicciones y creación de CSV para descarga
                df_res, csv = predObj.contac_prediccion(d_proc,
                                                        pred,
                                                        pred_sinesc)
                ...
                with col1:
                    ...
                    fig, ax = plt.subplots()
                    pd.Series(np.exp(pred)).hist(bins=50, ax=ax)
